...
def detectar_modo(messages):
    ...
    for modo, palabras in keywords.items():
        for palabra in palabras:
            if palabra in ultimo_msg:
                puntuacion[modo] += 1

    mejor_modo = max(puntuacion, key=puntuacion.get)
    return mejor_modo if puntuacion[mejor_modo] >= 1 else "general"
        ...
        if submitted:
            if not nombre.strip():
                ...
                if perfil_exist:
                    ...
                    manejador.guardar_perfil(alumno_id, perfil_nuevo)
                    st.session_state.vista = "chat"
                    st.rerun()
...
{"- Es alumno NUEVO, primera sesion." if perfil.get('total_sesiones', 1) == 1
 else f"- Alumno RECURRENTE con {perfil.get('total_sesiones',1)} sesiones."}
"""
...
# ════════════════════════════════════════════════════════════
    ...
    with st.sidebar:
        ...
        perfiles_todos     = manejador.listar_perfiles()
    ...
    perfiles = manejador.listar_perfiles()
    ...
    if not perfiles:
        ...
        <div class="card" style="text-align:center; padding:40px;">
            ...
            <p style="color:#94a3b8; font-size:0.9rem;">
                Los perfiles aparecen aquí cuando los alumnos inician sesión con Moni.
            </p>
        </div>
        """, unsafe_allow_html=True)
    else:
